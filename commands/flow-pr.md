...
1. Prepare: run the runtime preparation commands. Use the compact context to draft the small semantic intent, write it only to the exact runtime-created OS-temp `intentPath`, then finalize preparation with its handle. Never display the internal snapshot, request payload, temp path, or handle mechanics.
