...
Run the `/flow-commit` compact prepare, semantic intent, non-consuming validation, seal, and execute workflow for the current worktree only. Keep one specialized agent and one human mutation approval. Validation permits at most one correction of the same intent path for the explicit authoring allowlist, with no Git-fact reread or fresh prepare.
