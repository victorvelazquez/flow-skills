...
For genuine base or fork ambiguity, invoke OpenCode's `question` tool, wait for the user's answer inside this same child invocation, then continue preparation with that answer. Never finish or return a plain-text clarification question to the parent.
