...
Only when preparation returns `base-ambiguous` or fork semantics remain genuinely ambiguous, invoke OpenCode's `question` tool, wait for the user's exact base/remote answer inside this same child invocation, then continue preparation with that answer. If the question capability is unavailable, return `unavailable` without mutation. Never infer authority from branch names, topology, merge-base proximity, or ancestry, and never finish or return a plain-text clarification question to the parent.
