...
import {
  authorIntent,
  contentFactsFingerprint,
  executeHandle,
  prepareRepository,
  repositoryLockPath,
  sealHandle,
} from "../scripts/flow-commit.mjs";
...
function payload(prepared, units, branch = { action: "keep" }, extra = {}) {
  const ordinals = new Map(
    prepared.changes.map((change, index) => [change.path, index]),
  );
  const document = {
    schema: "flow-commit/author-intent-v1",
    ...(branch.action === "create" ? { branchName: branch.name } : {}),
    units: units.map(({ paths, ...unit }) => ({
      ordinals: paths.map((file) => ordinals.get(file)),
      ...unit,
    })),
    ...extra,
  };
...
function author(
  ...
  units,
  branch = { action: "keep" },
  token = payload(prepared, units, branch),
) {
...
function ready(cwd, units, branch = { action: "keep" }) {
  ...
  const sealed = seal(cwd, {
    ...prepared,
    handle: authored.output.authoredHandle,
  });
  assert.equal(sealed.result.status, 0, sealed.result.stdout);
  return {
    prepared: { ...prepared, handle: sealed.output.executeHandle },
    sealed: sealed.output,
  };
}
...
test("prepare is compact, NUL-safe, and covers hostile, untracked, and deleted paths", () => {
  ...
  for (const forbidden of [
    ...
    assert.equal(Object.hasOwn(prepared, forbidden), false);
...
test("prepare creates only immutable runtime-owned authority", () => {
  ...
  if (process.platform !== "win32")
    assert.equal(
      fs.statSync(path.join(store(prepared), "prepared.json")).mode & 0o777,
      0o600,
    );
...
test("runtime encoder constructs canonical payload and author, seal, execute complete the lifecycle", () => {
  ...
  const encoded = run(cwd, [
    ...
    "0",
    "--title",
    "fix(commit): author runtime payload",
  ]);
  ...
  const authored = run(cwd, [
    "--author-intent",
    "--handle",
    prepared.handle,
    "--payload-b64url",
    encodedOutput.payloadB64url,
  ]);
  ...
  const sealed = seal(cwd, {
    ...prepared,
    handle: authoredOutput.authoredHandle,
  });
  assert.equal(sealed.result.status, 0, sealed.result.stdout);
  const executed = execute(cwd, {
    ...prepared,
    handle: sealed.output.executeHandle,
  });
...
test("ordered ordinal units resolve to exact prepared paths", () => {
  ...
  assert.equal(sealed.result.status, 0, sealed.result.stdout);
  assert.deepEqual(
    sealed.output.units.map((unit) => unit.paths),
    [["one.txt"], ["two.txt"]],
  );
});
...
test("successful authoring is exclusive and exact replay is idempotent", () => {
  ...
  const token = payload(prepared, [
    { paths: ["change.txt"], title: "fix(commit): author once" },
  ]);
...
test("one invalid semantic payload is non-consuming and one correction can author", () => {
  ...
  const invalidToken = Buffer.from(
    canonical({
      schema: "flow-commit/author-intent-v1",
      units: [
        { ordinals: [1], title: "fix(commit): fail authoring validation" },
      ],
    }),
  ).toString("base64url");
  ...
  const corrected = author(cwd, prepared, [
    { paths: ["change.txt"], title: "fix(commit): correct structured intent" },
  ]);
  assert.equal(corrected.result.status, 0, corrected.result.stdout);
  assert.equal(store(prepared), originalStore);
  const sealed = seal(cwd, {
    ...prepared,
    handle: corrected.output.authoredHandle,
  });
  assert.equal(sealed.result.status, 0, sealed.result.stdout);
  const executed = execute(cwd, {
    ...prepared,
  ...
  assert.equal(
    git(cwd, ["log", "-1", "--format=%s"]),
    "fix(commit): correct structured intent",
  );
});
...
test("authored handle grants no execute authority", () => {
  ...
  fs.writeFileSync(path.join(cwd, "change.txt"), "change\n");
  const prepared = prepare(cwd);
  const authored = authorIntent(
    prepared.handle,
    payload(prepared, [
      { paths: ["change.txt"], title: "fix(commit): require sealed authority" },
    ]),
  );
  assert.equal(fs.existsSync(path.join(store(prepared), "sealed.json")), false);
  assert.throws(
    () => executeHandle(authored.authoredHandle),
    (error) => error.code === "handle-not-sealed",
  );
...
test("authoring rejects malformed transport variants and bounds one correction", () => {
  const variants = [
    ...
    Buffer.from(
      '{"units":[],"schema":"flow-commit/author-intent-v1"}',
    ).toString("base64url"),
  ];
  for (const token of variants) {
    ...
    assert.ok(
      ["invalid-payload", "invalid-intent"].includes(
...
test("authoring fails closed for prepared tamper, expiry, legacy handles, and invalid handles", () => {
  ...
  assert.equal(
    author(tamperRepo, prepared, [
      { paths: ["change.txt"], title: "fix(commit): reject prepared tamper" },
    ]).output.error.code,
    "prepared-tamper",
  );
  assert.equal(fs.existsSync(store(prepared)), false);
    ...
    () =>
      authorIntent(
        ...
        payload(prepared, [
          {
            paths: ["change.txt"],
            title: "fix(commit): reject expired author",
          },
        ]),
        { now: 1011 },
      ),
    (error) => error.code === "handle-expired",
  );
  assert.equal(fs.existsSync(store(prepared)), false);
  assert.equal(
    output(
  ...
  assert.equal(
    output(run(expiryRepo, ["--seal", "--handle", "not-a-handle"])).error.code,
    "invalid-handle",
  );
});
...
test("prepare rejects a preexisting index and repository operation without mutation", () => {
  ...
  const indexPath = git(stagedRepo, [
    "rev-parse",
  ...
  let response = run(stagedRepo, ["--prepare"]);
  let document = output(response);
  ...
  fs.writeFileSync(
    path.join(mergingRepo, ".git", "MERGE_HEAD"),
    `${"0".repeat(40)}\n`,
  );
  response = run(mergingRepo, ["--prepare"]);
...
test("seal detects changed bytes under the same path", () => {
  ...
  const prepared = prepare(cwd);
  const authored = author(cwd, prepared, [
    { paths: ["change.txt"], title: "fix(commit): bind changed bytes" },
  ]);
  fs.writeFileSync(path.join(cwd, "change.txt"), "two\n");
  const response = seal(cwd, {
    ...prepared,
    handle: authored.output.authoredHandle,
  });
  assert.equal(response.result.status, 1);
...
test("seal detects executable-mode and symlink-target drift", {
  skip:
    process.platform === "win32" &&
    "Windows fixture cannot reliably create POSIX symlinks and executable modes.",
}, () => {
  const modeRepo = repo();
  fs.writeFileSync(path.join(modeRepo, "mode.sh"), "exit 0\n", { mode: 0o644 });
  let prepared = prepare(modeRepo);
  let authored = author(modeRepo, prepared, [
    { paths: ["mode.sh"], title: "fix(commit): bind executable mode" },
  ]);
  fs.chmodSync(path.join(modeRepo, "mode.sh"), 0o755);
  assert.equal(
    seal(modeRepo, { ...prepared, handle: authored.output.authoredHandle })
      .output.error.code,
    "content-drift",
  );
  ...
  authored = author(linkRepo, prepared, [
    {
      paths: ["link", "one", "two"],
      title: "fix(commit): bind symlink target",
    },
  ]);
  fs.unlinkSync(path.join(linkRepo, "link"));
  fs.symlinkSync("two", path.join(linkRepo, "link"));
  assert.equal(
    seal(linkRepo, { ...prepared, handle: authored.output.authoredHandle })
      .output.error.code,
    "content-drift",
  );
});
...
test("content fingerprint binds executable mode and symlink target on every platform", () => {
  ...
  assert.notEqual(
    contentFactsFingerprint([file]),
    contentFactsFingerprint([{ ...file, mode: "100755" }]),
  );
  assert.notEqual(
    contentFactsFingerprint([link]),
    contentFactsFingerprint([{ ...link, bytes: 4, content: "c".repeat(64) }]),
  );
});
...
test("author strictly validates schema, coverage, and titles while seal returns a compact summary", () => {
  ...
  for (const [document, expected] of [
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [{ ordinals: [0], title: "fix(commit): miss coverage" }],
      },
      "coverage-mismatch",
    ],
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [{ ordinals: [0, 0], title: "fix(commit): repeat ordinal" }],
      },
      "coverage-mismatch",
    ],
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [{ ordinals: [0, 2], title: "fix(commit): reject range" }],
      },
      "coverage-mismatch",
    ],
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [{ ordinals: [0, 1], title: "not conventional" }],
      },
      "invalid-intent",
    ],
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [
          { ordinals: [0, 1], title: "feat!(commit): reject misplaced marker" },
        ],
      },
      "invalid-intent",
    ],
    [
      {
        ...
        units: [
          {
            ordinals: [0, 1],
            title: "fix(commit): reject extras",
            extra: true,
          },
        ],
      },
    ...
    [
      {
        schema: "flow-commit/author-intent-v1",
        units: [{ ordinals: [0, 1], title: "fix(commit): reject root extras" }],
        extra: true,
      },
      "invalid-intent",
    ],
    ...
    assert.equal(
      author(cwd, prepared, [], undefined, token).output.error.code,
      expected,
    );
  }
  ...
  const summary = seal(cwd, {
    ...prepared,
    handle: authored.output.authoredHandle,
  });
  assert.equal(summary.result.status, 0);
  assert.deepEqual(summary.output.repository, {
  ...
  assert.deepEqual(summary.output.units[0].body, {
    present: true,
    bytes: Buffer.byteLength(body),
  });
  assert.doesNotMatch(
    summary.result.stdout,
    /Useful context|fingerprint|snapshot|request-v3/,
  );
  ...
  assert.equal(
    author(
      collisionRepo,
      collision,
      [{ paths: ["change.txt"], title: "fix(commit): reject collision" }],
      { action: "create", name: "feat/collision" },
    ).output.error.code,
    "branch-collision",
  );
});
...
test("prepared authority digest and strict envelope block repository substitution", () => {
  ...
  let preparedA = prepare(repoA);
  let preparedB = prepare(repoB);
  const authoredA = author(repoA, preparedA, [
    { paths: ["a.txt"], title: "fix(commit): keep repository a" },
  ]);
  fs.copyFileSync(
    path.join(store(preparedB), "prepared.json"),
    path.join(store(preparedA), "prepared.json"),
  );
  let response = seal(repoA, {
    ...preparedA,
    handle: authoredA.output.authoredHandle,
  });
  assert.equal(response.result.status, 1);
  ...
  preparedA = ready(repoA, [
    { paths: ["a.txt"], title: "fix(commit): seal repository a" },
  ]).prepared;
  ...
  const strictAuthored = author(strictRepo, strictPrepared, [
    { paths: ["change.txt"], title: "fix(commit): reject prepared extras" },
  ]);
  ...
  assert.throws(
    () => sealHandle(forgedHandle),
    (error) => error.code === "prepared-tamper",
  );
...
test("sealed execute handle binds the exact approved request", () => {
  ...
  fs.writeFileSync(path.join(repoA, "a.txt"), "a\n");
  fs.writeFileSync(path.join(repoB, "b.txt"), "b\n");
  const preparedA = ready(repoA, [
    { paths: ["a.txt"], title: "fix(commit): approve repository a" },
  ]).prepared;
  const preparedB = ready(repoB, [
    { paths: ["b.txt"], title: "fix(commit): approve repository b" },
  ]).prepared;
  fs.copyFileSync(
    path.join(store(preparedB), "authored.json"),
    path.join(store(preparedA), "authored.json"),
  );
  fs.copyFileSync(
    path.join(store(preparedB), "sealed.json"),
    path.join(store(preparedA), "sealed.json"),
  );
...
test("authored tamper, expiry, consumed handle, and reuse fail closed", () => {
  const cwd = repo();
  fs.writeFileSync(path.join(cwd, "change.txt"), "change\n");
  let prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): reject authored tamper" },
  ]).prepared;
  ...
  const expiring = authorIntent(
    prepared.handle,
    payload(prepared, [
      { paths: ["change.txt"], title: "fix(commit): reject expiry" },
    ]),
    { now: 1001 },
  );
  assert.throws(
    () => sealHandle(expiring.authoredHandle, { now: 1011 }),
    (error) => error.code === "handle-expired",
  );

  prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): consume once" },
  ]).prepared;
...
test("execute hashing is linear and rereads only the current future unit", () => {
  ...
  const reads = [];
  const result = executeHandle(prepared.handle, {
    onContentRead: ({ path: file }) => reads.push(file),
  });
  assert.equal(result.status, "success");
  const counts = Object.fromEntries(
    units.map((unit) => [
      unit.paths[0],
      reads.filter((file) => file === unit.paths[0]).length,
    ]),
  );
...
test("symbolic-link handle stores fail closed", {
  skip:
    process.platform === "win32" &&
    "Windows symlink creation requires elevated fixture privileges.",
}, () => {
  const cwd = repo();
  fs.writeFileSync(path.join(cwd, "change.txt"), "change\n");
  const prepared = prepare(cwd);
  const unsafe = store(prepared);
  const moved = `${unsafe}-real`;
  fs.renameSync(unsafe, moved);
  fs.symlinkSync(moved, unsafe, "dir");
...
test("same-handle concurrent claim allows exactly one mutation", async () => {
  ...
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): claim once" },
  ]).prepared;
  ...
  assert.equal(
    documents.filter(
      (item) =>
        item.error?.code === "handle-claimed" ||
        item.error?.code === "handle-unavailable",
    ).length,
    1,
  );
...
test("distinct handles for one common-dir allow exactly one execution to mutate", async () => {
  ...
  fs.writeFileSync(path.join(cwd, "change.txt"), "change\n");
  hook(cwd, "pre-commit", "sleep 1\nexit 0");
  const first = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): first authority" },
  ]).prepared;
  const second = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): second authority" },
  ]).prepared;
  const responses = await Promise.all([
  ...
  assert.equal(documents.filter((item) => item.status === "success").length, 1);
  assert.equal(
    documents.filter((item) => item.error?.code === "repository-locked").length,
    1,
  );
  assert.equal(git(cwd, ["rev-list", "--count", "HEAD"]), "2");
});
...
test("abandoned handle claim and repository lock fail closed", () => {
  ...
  let prepared = ready(claimedRepo, [
    { paths: ["change.txt"], title: "fix(commit): reject abandoned claim" },
  ]).prepared;
  fs.writeFileSync(path.join(store(prepared), "execute.claim"), "abandoned\n", {
    flag: "wx",
  });
  assert.equal(
    execute(claimedRepo, prepared).output.error.code,
    "handle-claimed",
  );

  const lockedRepo = repo();
  fs.writeFileSync(path.join(lockedRepo, "change.txt"), "change\n");
  prepared = ready(lockedRepo, [
    { paths: ["change.txt"], title: "fix(commit): reject abandoned lock" },
  ]).prepared;
...
test("index restoration refuses to overwrite hook-created foreign staging", () => {
  ...
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): preserve foreign staging" },
  ]).prepared;
  const response = execute(cwd, prepared);
...
test("first-unit failure after branch creation is partial with explicit branch effect", () => {
  ...
  const prepared = ready(
    cwd,
    [{ paths: ["change.txt"], title: "fix(commit): report created branch" }],
    { action: "create", name: "fix/created-effect" },
  ).prepared;
  const response = execute(cwd, prepared);
  ...
  assert.equal(
    git(cwd, [
      ...
      "--local",
      "--get",
      "branch.fix/created-effect.gh-merge-base",
    ]),
    "main",
  );
  assert.deepEqual(response.output.stoppedAt.paths, ["change.txt"]);
...
test("branch provenance failure rolls back the branch and stale config", () => {
  ...
  const plan = ready(
    cwd,
    [{ paths: ["change.txt"], title: "fix(commit): rollback provenance" }],
    { action: "create", name: "fix/provenance-rollback" },
  ).prepared;
  const result = executeHandle(plan.handle, {
  ...
  assert.equal(
    maybeGit(cwd, [
      "show-ref",
      "--verify",
      "refs/heads/fix/provenance-rollback",
    ]),
    "",
  );
  assert.equal(
    maybeGit(cwd, [
      ...
      "--local",
      "--get",
      "branch.fix/provenance-rollback.gh-merge-base",
    ]),
    "",
  );
});
...
test("branch-create blocker leaves every unit not attempted", () => {
  ...
  const prepared = ready(
    ...
    { action: "create", name: "feat/collided-after-seal" },
  ).prepared;
  ...
  assert.deepEqual(
    response.output.notAttempted.map((unit) => unit.paths),
    [["one.txt"], ["two.txt"]],
  );
...
test("subdirectory invocation normalizes all Git work to the canonical root", () => {
  ...
  const prepared = ready(subdir, [
    {
      paths: ["nested/change.txt"],
      title: "fix(commit): normalize repository root",
    },
  ]).prepared;
  ...
  assert.equal(
    git(cwd, ["log", "-1", "--format=%s"]),
    "fix(commit): normalize repository root",
  );
  if (process.platform === "win32") {
    fs.writeFileSync(path.join(subdir, "case.txt"), "case\n");
    const alias = subdir.toUpperCase();
    const casePrepared = ready(alias, [
      {
        paths: ["nested/case.txt"],
        title: "fix(commit): normalize root casing",
      },
    ]).prepared;
...
test("CLI rejects unknown, duplicate, missing, and incompatible options", () => {
  ...
  for (const [args, pattern] of [
    ...
    [
      ["--prepare", "--execute", "--handle", "a".repeat(64)],
    ...
    [
      ["--author-intent", "--handle", "a".repeat(64)],
      /CLI options are invalid/i,
    ],
    [["--execute"], /requires --handle/],
    [["--prepare", "--handle", "x"], /accepts no handle/i],
    [
      ["--execute", "--handle", "a".repeat(64), "--handle", "b".repeat(64)],
      /Duplicate option/,
    ],
  ]) {
    const response = run(cwd, args);
    const document = output(response);
    assert.equal(response.status, 1);
...
test("ordered units retain commit verification and compact success omits sensitive payloads", () => {
  ...
  assert.equal(response.output.status, "success");
  assert.deepEqual(
    response.output.completed.map((unit) => unit.title),
    ["feat(commit)!: create first unit", "fix(commit): create second unit"],
  );
  assert.equal(response.output.stoppedAt, null);
  assert.deepEqual(response.output.notAttempted, []);
  assert.deepEqual(response.output.outstandingPaths, []);
  assert.deepEqual(response.output.counts, {
    completed: 2,
    notAttempted: 0,
    outstandingPaths: 0,
    leftovers: 0,
  ...
  assert.doesNotMatch(
    ...
    /Why this unit exists|request-v3|snapshot|fingerprint|"paths"/,
  );
  assert.equal(
    git(cwd, ["log", "-1", "--format=%s"]),
    "fix(commit): create second unit",
  );
  const firstCommit = git(cwd, ["rev-parse", "HEAD~1"]);
  const commitObject = execFileSync(
    "git",
    ["cat-file", "commit", firstCommit],
    { cwd, encoding: "utf8" },
  );
  assert.equal(
    commitObject.slice(commitObject.indexOf("\n\n") + 2),
    `feat(commit)!: create first unit\n\n${body}\n`,
  );
});
...
test("later hook failure preserves completed units and actionable remaining paths", () => {
  ...
  const prepared = ready(cwd, [
    ...
    {
      paths: ["two.txt"],
      title: "fix(commit): fail second unit",
      body: "Not repeated in output.",
    },
    {
      paths: ["three.txt"],
      title: "docs(commit): leave third unit unattempted",
    },
  ]).prepared;
  const response = execute(cwd, prepared);
  assert.equal(response.result.status, 2);
...
test("hook path effects trigger CAS rollback and observable-effect reporting", () => {
  ...
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): reject hook path effect" },
  ]).prepared;
  const response = execute(cwd, prepared);
  assert.notEqual(response.result.status, 0);
  assert.match(
    response.output.error.message,
    /postcondition|parent, tree, paths/i,
  );
...
test("post-commit concurrent HEAD is preserved and never globally rolled back", () => {
  ...
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): preserve concurrent head" },
  ]).prepared;
  const response = execute(cwd, prepared);
  assert.equal(response.output.status, "drift");
...
test("handle stores are cleaned after ordinary failure", () => {
  ...
  hook(cwd, "pre-commit", "exit 1");
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): clean failed handle" },
  ]).prepared;
  const response = execute(cwd, prepared);
  assert.equal(response.result.status, 1);
  assert.equal(fs.existsSync(store(prepared)), false);
...
test("index restoration accepts hook-restaged sealed content", () => {
  const cwd = repo();
  fs.writeFileSync(path.join(cwd, "change.txt"), "change\n");
  hook(
    cwd,
    "pre-commit",
    "git reset -- change.txt\ngit add -- change.txt\nexit 1",
  );
  const prepared = ready(cwd, [
    { paths: ["change.txt"], title: "fix(commit): restore sealed staging" },
  ]).prepared;
  const response = execute(cwd, prepared);
  assert.equal(response.result.status, 1, response.result.stdout);
  assert.equal(response.output.status, "failure");
  assert.equal(response.output.error.code, "commit-failed");
  assert.deepEqual(
