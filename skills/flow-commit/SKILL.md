---
name: flow-commit
description: "Prepare semantic commit units and execute one sealed, verified Git transaction."
license: Apache-2.0
metadata:
...
Use this workflow to commit current local changes through Flow. Resolve the packaged `../../scripts/flow-commit.mjs` runtime relative to this skill; it is the Git execution source of truth.

## Hard Rules

- Keep one specialized workflow session. It reads necessary diffs, chooses ordered semantic units, and never delegates or sends full diff/request context through another agent.
