...
- The host's execute approval is the one human mutation approval. In Pi, the `/flow-pr` extension command presents the finalized runtime approval summary through one native `ctx.ui.confirm` immediately before execute. The command invocation itself is not consent, regardless of whether it came from a TUI editor, startup input, or an extension. Decline, absent UI, timeout, or uncertain response stops without execute. Pi delegated agents and prompt templates must be mechanically unable to execute publication; natural-language requests, delegated subagents, model-visible messages, and adapter metadata never grant publication approval. Never ask for a separate conversational confirmation after a valid host boundary. For genuine base or fork ambiguity, the adapter obtains an answer within its active interaction and continues preparation; if that capability is unavailable, return `unavailable` without mutation.
...
| One safe repository template is available | Other semantic adapters may preserve its structure with validated claims; Pi direct uses a conservative generic body with `Not provided` validation instead of importing template claims. |
