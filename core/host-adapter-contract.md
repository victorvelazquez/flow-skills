...
Mutation requires host-native approval and revalidation of workflow-owned immutable input before execution. Exceptions are operation-scoped: manual `/flow-commit` and `/flow-pr` invocation authorizes their respective local commit and PR push/create/update; explicit Flow Branch selection (manual branch name or number resolved from the current runtime inventory) authorizes only the existing direct checkout and fast-forward-only update, without a second `ask-pull`. Flow Branch deletion still requires separate confirmation, and force deletion requires per-branch approval. Other operations retain their host-native gate. Adapters own interaction and presentation. Shared Flow does not prescribe universal permission APIs, approval tokens, review transactions, host commands, workflow taxonomies, storage models, or serialized schemas.
...
7. **Authorize** a mutation through a host-native gate, except the operation-scoped invocation and explicit selection authorizations named above. Discovery, invocation of other workflows, previous answers, and adapter metadata never grant consent.
...
- An unavailable required clarification or approval capability stops the affected mutation clearly; Flow Commit/PR need no second gate for their authorized operations; Flow Branch explicit selection needs no second update gate, but deletion remains gated.
